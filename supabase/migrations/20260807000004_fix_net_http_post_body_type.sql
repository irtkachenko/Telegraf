-- Fix error: function net.http_post(url => text, headers => jsonb, body => text) does not exist
-- pg_net's net.http_post expects body => jsonb, NOT body => text
-- The previous migration (20260807000003) used body := payload::text which casts jsonb to text,
-- causing PostgreSQL to fail finding a matching function overload.

-- Recreate send_push_notification with correct body type (jsonb, not text)
