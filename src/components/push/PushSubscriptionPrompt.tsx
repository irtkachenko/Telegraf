 ...
 * no subscription exists). Works on both mobile (PWA) and desktop.
 */
export default function PushSubscriptionPrompt() {
  const { isSubscribed, isCheckingSubscription, isSubscribing, pushSupported, subscribe } =
  ...
  // Don't show on landing page where user is not logged in
  const isDenied =
    typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'denied';

  // Don't show if already subscribed, checking, not supported, denied permission,
  // or dismissed this session
  if (isSubscribed || isCheckingSubscription || !pushSupported || isDenied || dismissed) return null;

  // Don't show again this session if user dismissed
  if (typeof window !== 'undefined' && sessionStorage.getItem(PROMPT_SESSION_KEY)) return null;
